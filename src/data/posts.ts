export interface Post {
  ...
  readTime: string
  date: string
  content: string // markdown-style sections as HTML string
}
...
export const posts: Post[] = [
  {
    ...
    excerpt:
      'Walking through the architecture, training challenges, and deployment of a GAN that synthesizes text, imaging descriptors, and vitals — achieving ~40% training data diversity improvement.',
...
<ul>
  ...
  },
  {
    ...
    excerpt:
      'How I redesigned a serial EHR ingestion pipeline into a distributed system that processes 10 GB of Electronic Health Records 3x faster — and what I learned about bottlenecks along the way.',
...
<p>This pipeline feeds the <strong>SynMedix AI platform</strong>, which uses the processed, clean EHR structure to train generative models and produce 50,000+ synthetic patient records.</p>
...
<ul>
  ...
  },
  {
    ...
    excerpt:
      'A practical walkthrough of designing, building, and deploying an Apache Airflow DAG to automate AMFI mutual fund regulatory data ingestion — including the mistakes that cost me two days.',
    tag: 'Data Engineering',
    color: '#f59e0b',
    readTime: '8 min read',
    date: 'Mar 2025',
...
<h2>Outcome</h2>
<p>The pipeline runs daily at 6:00 AM, processes AMFI data automatically, and has run reliably since deployment. Manual data preparation time dropped to near zero. The Power BI dashboards now update automatically for stakeholders every morning.</p>
    `.trim(),
  },
  {
    ...
    excerpt:
      'How I built a real-time home energy management system using embedded hardware, Python, and a web dashboard — and why IoT projects are the best way to learn full-stack engineering.',
    tag: 'IoT',
    color: '#00d4ff',
    readTime: '7 min read',
    date: 'Aug 2023',
...
<p>The project was my IoT internship deliverable at Intuz Solution, but it quickly became one of my favorite builds — hardware plus firmware plus backend plus frontend in a single project.</p>
...
<ul>
  ...
  },
  {
    ...
    excerpt:
      'The academic, cultural, and professional differences I encountered moving from a B.Tech in India to an M.S. CS program in the US — and what I wish I had known before applying.',
    tag: 'Personal',
    color: '#ef4444',
    readTime: '6 min read',
    date: 'Jun 2025',
...
<p>If you're a student considering this path — feel free to reach out via LinkedIn or email. Happy to share more specifics.</p>
    `.trim(),
  },
  {
    ...
    excerpt:
      'Everything I learned deploying a multimodal GAN to GCP Vertex AI — from containerizing the model to setting up endpoints, monitoring, and keeping costs sane.',
    tag: 'Cloud / MLOps',
    color: '#3b82f6',
...
<p>That said, I've deployed on SageMaker too (for SynMedix), and both are mature platforms. The concepts transfer — the APIs differ.</p>

<h2>Step 1: Containerize Your Model</h2>
<p>Vertex AI serves models as containers. You need a Docker image that:</p>
<ol>
  <li>Installs your dependencies (PyTorch, NumPy, your custom code)</li>
  <li>Exposes an HTTP server on port 8080</li>
  <li>Handles <code>/health</code> and <code>/predict</code> routes</li>
</ol>
...
<p>Cold start time (container startup + model load) was about 90 seconds. For a research tool where latency isn't critical, this was acceptable.</p>
...
<h2>Monitoring</h2>
<p>Vertex AI automatically logs prediction requests and latency to Cloud Monitoring. I added custom metrics for model-specific things: generation time per modality, cross-modal consistency score, and output diversity. These feed into a simple Cloud Monitoring dashboard I check weekly.</p>

<h2>The Full Picture</h2>
<p>Vertex AI is a genuinely good platform once you get past the initial setup friction. The biggest time sinks were: IAM permissions (always), container debugging (test locally with Docker first), and understanding the difference between Model, Endpoint, and DeployedModel resources (they are distinct things, and the naming is confusing).</p>
