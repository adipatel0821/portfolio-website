  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (p > 0.05) {
        ...
        g.addColorStop(0, `rgba(168,85,247,${glowAlpha})`)
      ...
      for (let i = 0; i < visCount; i++) {
        ...
        if (i > 0 && i % 6 === 0 && p > 0.3) {
          ...
          if (dist < 80) {
            ctx.strokeStyle = `rgba(168,85,247,${0.12 * appear * p})`
            ctx.lineWidth = 0.4
            ctx.beginPath()
            ctx.moveTo(x, y)
            ctx.lineTo(px2, py2)
        ...
        col.addColorStop(0, `hsla(${pt.hue},90%,72%,${appear * depth})`)
        col.addColorStop(1, `hsla(${pt.hue},90%,72%,0)`)
        ctx.fillStyle = col
        ctx.beginPath()
        ctx.arc(x, y, r * 4, 0, Math.PI * 2)
        ctx.fill()

        ctx.fillStyle = `hsla(${pt.hue},100%,85%,${appear * depth * 0.9})`
        ctx.beginPath()
